...
{
    [self makeFireworksWithBirthRate:0.0 andEmitter:emitter];
}
...
- (void)makeFireworksWithBirthRate:(CGFloat)birthrate andEmitter:(CAEmitterLayer *)emitter
{
	...
	emitter.emitterPosition = CGPointMake(viewBounds.size.width/2.0, viewBounds.size.height);
	emitter.emitterSize	= CGSizeMake(viewBounds.size.width/2.0, 0.0);
	...
	burst.birthRate			= birthrate;		// at the end of travel
	burst.velocity			= 0;
	burst.scale				= 2.5;
	...
	// putting it together
	emitter.emitterCells	= [NSArray arrayWithObject:rocket];
